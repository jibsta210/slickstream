...
class SportsRepositoryImpl(
    ...
    override suspend fun categories(): DataResult<List<SportCategory>> = guard {
        ...
        // FanCode is dropped: its feeds are Google-DAI HLS geo-locked to India, so they 403 abroad
        // (every non-India user just gets dead cricket/football tabs). streamed.pk covers the other
        // sports (football/cricket/tennis/…) via the embed resolver, so we rely on it for everything.
        ...
        // Live now first (the cross-sport live aggregate), then the leagues (MLB/NBA/NHL/NFL first).
        (listOf(live) + streamedCats).distinctBy { it.id }
    }
    ...
    override suspend fun events(categoryId: String): DataResult<List<SportEvent>> = guard {
        ...
        when {
            categoryId == SportsRepository.LIVE_CATEGORY_ID -> {
                // streamed.pk only (FanCode dropped — geo-locked to India).
                if (!streamedEnabled) emptyList() else
